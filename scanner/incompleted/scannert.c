...
void skipComment() {
  // loai bo comment
  while((currentChar != EOF) && (charCodes[currentChar] == CHAR_TIMES) && (charCodes[currentChar] == CHAR_RPAR))
    readChar();
}
...
Token* readIdentKeyword(void) {
  // TODO
  int count =0;
  Token *token = makeToken(TK_IDENT, lineNo, colNo); //tao 1 token moi
...
Token* readNumber(void) {
  ...
  token->string[count] ='\0';
  token ->value = atoi(token->string);// chuyen so thanh chuoi
  return token;
}
