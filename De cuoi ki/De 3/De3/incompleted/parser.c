...
void compileVariables(List *list) {
    if (lookAhead->tokenType == SB_COMMA) {
        eat(SB_COMMA);
        compileVariable(list);
    }
}

void compileVariable(List *list) {
    eat(TK_IDENT);
    checkFreshIdent(currentToken->string);
    List *node = (List *) calloc(1, sizeof(List *));
    node->next = NULL;
    node->node = createVariableObject(currentToken->string);
    list->next = node;
    compileVariables(list->next);
}
...
void compileBlock3(void) {
    ...
    if (lookAhead->tokenType == KW_VAR) {
        ...
        do {
            ...
            list = list->next;
            while (list != NULL) {
                list->node->varAttrs->type = varType;
                declareObject(list->node);
                list = list->next;
            }
            eat(SB_SEMICOLON);
        } while (lookAhead->tokenType == TK_IDENT);
