...
void compileAssignSt(void) {
    List *listL = (List *) calloc(1, sizeof(List *));
    List *listR = (List *) calloc(1, sizeof(List *));
    compileAssignStLeft(listL);
    eat(SB_ASSIGN);
    compileAssignStRight(listR);
    listL = listL->next;
    listR = listR->next;
    while (listR != NULL && listL != NULL) {
        checkTypeEquality(listL->type, listR->type);
        listL = listL->next;
        listR = listR->next;
    }
...
void compileAssignStLeft(List *list) {
    List *node = (List *) calloc(1, sizeof(List *));
    node->type = compileLValue();
    node->next = NULL;
    list->next = node;
    compileAssignStLefts(list->next);
}

void compileAssignStLefts(List *list) {
    if (lookAhead->tokenType == SB_COMMA) {
        eat(SB_COMMA);
        compileAssignStLeft(list);
    }
}

void compileAssignStRight(List *list) {
    List *node = (List *) calloc(1, sizeof(List *));
    node->type = compileExpression();
    node->next = NULL;
    list->next = node;
    compileAssignStRights(list->next);
}

void compileAssignStRights(List *list) {
    if (lookAhead->tokenType == SB_COMMA) {
        eat(SB_COMMA);
        compileAssignStRight(list);
    }
}
...
void compileExpression3(Type *typ) {
    Type *type;
