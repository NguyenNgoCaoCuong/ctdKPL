...
void compileAssignSt(void) {
    int total = 1;
    typedef Type *TypeP;
    TypeP *listTypeL = (TypeP *) calloc(1, sizeof(TypeP *));
    listTypeL[0] = compileLValue();
    while (lookAhead->tokenType == SB_COMMA) {
        eat(SB_COMMA);
        listTypeL = (TypeP *) realloc(listTypeL, (total++) * sizeof(TypeP *));
        listTypeL[total - 1] = compileLValue();
    }
    eat(SB_ASSIGN);
    checkTypeEquality(listTypeL[0], compileExpression());

    for (int i = 1; i < total; ++i) {
        if (lookAhead->tokenType == SB_COMMA)
            eat(SB_COMMA);
        else error(ERR_INVALID_STATEMENT, lookAhead->lineNo, lookAhead->colNo);
        checkTypeEquality(listTypeL[i], compileExpression());
    }
}
