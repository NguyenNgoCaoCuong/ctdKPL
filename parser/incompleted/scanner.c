...
Token *getValidToken(void) {
    Token *token = getToken();// đọc một token từ file đầu vào
    while (token->tokenType == TK_NONE) {// nếu token đọc được là TK_NONE thì đọc vào token mới
        free(token);
        token = getToken();
    }
    return token;// trả về một token != TK_NONE
}
